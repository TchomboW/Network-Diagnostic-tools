// utils/dns_cache_final_debug3.go - Fixed version with correct map access patterns
package main
...
func main() {
	...
	cache.mu.RLock()
	entryBefore, existsBefore := cache.entries["8.8.8.8"]
	cache.mu.RUnlock()
	fmt.Printf("   \"8.8.8.8\" exists before first resolve: %v\n", existsBefore)
	if existsBefore {
		fmt.Printf("   Entry data: %+v\n", entryBefore)
	} else {
		fmt.Println("   (No existing cache entry, as expected)")
	}
	...
	cache.mu.RLock()
	entryAfter1, existsAfter1 := cache.entries["8.8.8.8"]
	cache.mu.RUnlock()
	fmt.Printf("\n3. After first resolve:\n")
	if !existsAfter1 {
		fmt.Println("❌ Entry doesn't exist after first resolution but we got an IP!")
		return
	}
	...
	if hit1 {
		...
		fmt.Println("\n❌ First resolution returned HIT when it should have been MISS!")
	...
	cache.mu.RLock()
	entryAfterWait, existsAfterWait := cache.entries["8.8.8.8"]
	cache.mu.RUnlock()
	...
	if existsAfterWait && timeSince > currentTTL {
